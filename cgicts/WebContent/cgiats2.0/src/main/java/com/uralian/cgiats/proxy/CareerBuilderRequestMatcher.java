...
/**
 * 
 * Request matcher to decide when Career builder download process needs to be
 ...
 * @author jbernalm
 */
public class CareerBuilderRequestMatcher implements HttpRequestMatcher {

	private static final Logger log = Logger.getLogger(CareerBuilderRequestMatcher.class);

	@Override
	public boolean shouldFilterResponses(HttpRequest request) {

		log.info("Start From Career Builder RequestMatcher");
		...
		if(uri.toLowerCase().contains("http://employer.careerbuilder.com/jobposter/resumes/ResumeDetails.aspx".toLowerCase())){
			return true;
		}
		log.info("End From Career Builder RequestMatcher");
		return false;
	}
}
