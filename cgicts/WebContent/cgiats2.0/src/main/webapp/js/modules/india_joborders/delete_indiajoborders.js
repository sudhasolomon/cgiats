;(function(angular){
	...
	angular.module('deleteIndiaJobOrders',['ngMaterial', 'ngMessages','ui.bootstrap','jcs-autoValidate', 'DatePicker'])
	
	.controller("deleteIndiaJobOrdersController",function($rootScope, $scope, blockUI, $http, $timeout, $filter, $mdDialog, $mdMedia, $window, $state){
		
		$scope.candidates = [];
		$scope.candidates.push({});
    	var candidateData = [];
    	$scope.DeletedMyIndiaJobOrderTable = false;
    	$scope.DeletedAllIndiaJobOrderTable = false;
    	
    	$scope.onload = function(){
    		$scope.myIndiajobordersreset = function()
			{
				var priorityval = "High, Medium, Low";
		    	var statusval = "Open, Assigned, Reopen, Hold";
		...
		$scope.allIndiajobordersreset = function()
		{
			var priorityval = "High, Medium, Low";
	    	var statusval = "Open, Assigned, Reopen, Hold";
	    	var jobtypeval = "Permanent, Contract, Both, Not Specified";
		...
		$scope.myIndiaJobOrders = function()
		{
			findDeletedMyJobOrders();
		}
...
//         	alert(JSON.stringify($scope.searchFields));
			var response = $http.post("India_JobOrder/getAllIndiaDeletedMyJobOrders",$scope.searchFields);
			...
			$(".underlay").hide();
			 $scope.DeletedMyIndiaJobOrderTable = true;
		}
		...
		function dispalyTable() {
			$scope.candidates
					.forEach(function(candidate, index) {
						candidate.deletedMyIndiaBsTableControl = {
							options : {
								data : candidateData || {},
								striped : true,
								pagination : true,
								...
								columns : [
										...
										},*/
										{
											...
											align : 'left',
											sortable : false,
											events : window.myIndiaReopenOperateEvents,
						...
						window.myIndiaReopenOperateEvents = {
						...
						function DialogController($scope,
	                      ...
	                      $scope.reopenJobOrder = function() {
					    		  ...
					    		  var response = $http.get('India_JobOrder/reopenIndiaJobOrder/'+jobordid);
					   	          response.success(function(data, status, headers, config) {
//					   	        	findDeletedMyJobOrders();
					   	        	angular.element("#deleteIndiaJobOrdersControllerId").scope().myIndiaJobOrders();
		...
		$scope.allIndiaJoborders = function(){
			findAllDeletedJoborders();
		}
		...
		function findAllDeletedJoborders(){
			...
			var response = $http.post("India_JobOrder/getAllIndiaDeletedJobOrders",$scope.searchFields);
			response.success(function(data, status,headers, config) 
					{
			 candidateData = [];
				for (var i = 0; i < data.length; i++) {
			...
			$scope.DeletedAllIndiaJobOrderTable = true;
		}
		...
		function dispalyTable1() {
			$scope.candidates
					.forEach(function(candidate, index) {
						candidate.deletedAllIndiaBsTableControl = {
							options : {
								data : candidateData || {},
								striped : true,
								pagination : true,
...
/*						function DialogController($scope,
	                      ...
	                      $scope.reopenJobOrder = function() {
					    		  ...
					    		  var response = $http.get('indiaJobOrder/reopenIndiaJobOrder/'+jobordid);
					   	          response.success(function(data, status, headers, config) {
					   	        	findAllDeletedJoborders();
					   	     		$.growl	.notice({title : "Reopened", message : rowData.jobOrderId + " reopened successfully "});
					   	  		   });
