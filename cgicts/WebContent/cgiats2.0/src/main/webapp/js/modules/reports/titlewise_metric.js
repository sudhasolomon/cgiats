;(function(angular){
	angular.module("titlewisemetricModule",['jcs-autoValidate'])
	.controller("titlewisemetriccontroller", function($scope,$http,$state,$timeout,$mdDialog,$stateParams, $sessionStorage){
...
/*-***********************************DATA for REPORT FUNCTION STARTS HERE*******************************************-*/
		$scope.getGridData = function(){
			/*$scope.obj = {year: $scope.filterdetails.year, month: $scope.filterdetails.month , status: $scope.filterdetails.status , name :$scope.filterdetails.name};
			alert(JSON.stringify($scope.obj));
			var response = $http.post('totalReportController/getAllTitlesByClient',$scope.obj);*/
			$(".underlay").show();
			$scope.obj = {year: $scope.filterdetails.year, month: $scope.filterdetails.month , status: $scope.filterdetails.status , name :$scope.filterdetails.name};
//			alert(JSON.stringify($scope.obj));
			var response = $http.post('totalReportController/getAllTitlesByClient',$scope.obj);
					...
					$scope.totalData = data.data;
					 ...
					 var chart;
					   ...
					   $(document).ready(function () {
					       ...
					       {
					    	   title:{
					    		   text: '<b class="charttitle">Job Titles for  '+$scope.filterdetails.name+'</b>'
					    	   },
					    	   plotOptions: {
					               pie: {
