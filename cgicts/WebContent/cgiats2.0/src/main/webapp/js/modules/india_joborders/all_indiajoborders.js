;(function(angular){
	...
	angular.module('allIndiaJobOrders',['ngMaterial', 'ngMessages','ui.bootstrap','jcs-autoValidate', 'DatePicker','ngJsonExportExcel'])
	
	.controller("allIndiaJobOrdersController",function($rootScope, $scope, blockUI, $http, $timeout, $filter, $mdDialog, $mdMedia, $window, $state,dateRangeService){
		$('.ranges').empty();
...
//			alert($('#allJobOrderRangesId').val());
			...
			var response = $http.post("India_JobOrder/getAllIndiaJobOrders",$scope.searchFields);
			...
			$scope.candidates
					.forEach(function(candidate, index) {
											...
											response
													.success(function(
															...
															for (var i = 0; i < expansionData.length; i++) {
																tableres += '<tr><td>'
																		...
																		if(row.status!=constants.CLOSED){
																			...
																			+    '<a class="edit actionIcons" title="Edit" href="#/india_submitals/edit_indiasubmitals?submittalId='+expansionData[i].submittalId+'&pageName='+constants.ALLINDIAJOBORDERS+'"><i class="fa fa-edit" style="font-size:12px;"></i></a>'
																		...
																		+'<script type="text/javascript">'
																		 ...
																		 +'success:function(data){'
																		 +'$("#submitaldelete").hide();'
																		 +'angular.element("#allIndiaJobOrdersController").scope().searchjoborder();'
																		 +'angular.element("#allIndiaJobOrdersController").scope().$apply() '
																		 +'}'
																		 +'});'
																		 +'}'
																		 +'else{'
														...
														/*tableres += '</table><script type="text/javascript">function viewResumes(id){if((document.getElementById(id.id).value)==0){ $.growl	.error({message : "Select SearchBy Field for Viewing  Matching Resumes"});} else{ window.location="#/candidates/search?paramValue="+document.getElementById(id.id).value}}</script>'
															 ...
															 +'window.location="#/india_submitals/create_indiasubmitals?jobOrder="+job_orderId+"&pageName="+constants.ALLINDIAJOBORDERS'
															 +'}else{'
						...
						function actionFormatter(value, row,
								index) {
							//alert($rootScope.rsLoginUser.userRole);
