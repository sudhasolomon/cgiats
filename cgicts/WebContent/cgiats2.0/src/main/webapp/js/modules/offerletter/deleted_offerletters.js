;(function(angular){
	...
	angular.module("deletedOfferLetters",['ngMaterial', 'ngMessages','ui.bootstrap','jcs-autoValidate', 'DatePicker','ngJsonExportExcel'])
    .controller("deletedOfferLetterscontroller",function($rootScope, $scope, blockUI, $http, $timeout, $filter, $mdDialog, $mdMedia, $window, $state,mailService,dateRangeService) {
...
//			alert(JSON.stringify($scope.Created));
			...
			if($('#deletedOfferLetterRangesId').val() && $('#deletedOfferLetterRangesId').val()!=''){
				$scope.searchFields.timeIntervalMap.startDate = $('#deletedOfferLetterRangesId').val().split(' ')[0];
				$scope.searchFields.timeIntervalMap.endDate = $('#deletedOfferLetterRangesId').val().split(' ')[2];
			}
			...
			response.success(function(data, status,headers, config) 
					{
				//alert("data"+data);
				$scope.deletedOfferLettersList = data;
		...
		function dispalyTable() {
			$scope.offerLetterReports
					.forEach(function(candidate, index) {
						candidate.deletedOfferLetterBsTableControl = {
							options : {
						...
						function myActionFormatter(value, row,
							...
							return [
									'<a class="view actionIcons" title="Reopen JobOrder" onclick="viewReason(\''+row.reason+'\')"><i class="fa fa-search" style="font-size:12px;"></i></a> '
									+'<script type="text/javascript">function viewReason(reason){'
									+'$("#deletesubmittal").html(reason);'
									+'$("#submitaldelete").show();'
									+'}'
									+'</script>'
									 ]
